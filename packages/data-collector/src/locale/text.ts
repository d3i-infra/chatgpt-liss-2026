...
/**
 * `resolveAll` for the flat `{ <locale>: string }` shape — the form the
 * visualization components declare their own UI strings in, inline. Same
 * totality guarantee: a non-record container yields an empty table.
 */
export function resolveAllFlat (
  bundles: unknown,
  locale: string
): Record<string, string> {
  if (!isRecord(bundles)) return {}
  const resolved: Record<string, string> = {}
  for (const [key, value] of Object.entries(bundles)) {
    resolved[key] = resolveFlatText(value, locale)
  }
  return resolved
}
