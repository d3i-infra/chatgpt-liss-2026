...
describe('zTable', () => {
  ...
  // originalBody is part of the schema (the conversation view needs the
  // pristine row set to render deleted messages as placeholders), and shares
  // body's by-reference schema so it costs no extra row allocations.
  it('passes originalBody through by reference', () => {
    const input = makeTable()
    input.originalBody = { rows: [{ id: 'r1', cells: ['1', '2'] }] }
    const result = zTable.safeParse(input)
    expect(result.success).toBe(true)
    if (result.success) {
      expect(result.data.originalBody?.rows).toBe(input.originalBody.rows)
    }
  })

  it('accepts a table without originalBody', () => {
    const { originalBody, ...withoutOriginal } = makeTable()
    const result = zTable.safeParse(withoutOriginal)
    expect(result.success).toBe(true)
    if (result.success) expect(result.data.originalBody).toBeUndefined()
  })
