...
const zTableBody = z.object({
  rows: z.custom<TableRow[]>(isRowArray, { message: 'rows must be an array of { id: string, cells: string[] }' })
})
...
export const zTable = z.object({
  ...
  // Shares zTableBody, so it is passed through by reference like `body`: it
  // aliases the same row objects and adds no allocations of its own (#122).
